...
void MainWindow::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    ...
    ui->listWidget->addItem(new QListWidgetItem(QString::fromStdString(to_string(source + 1) + " owes " + to_string(target + 1) + " the amount " + to_string(debt))));
}
...
{
    vector<int> parent = tg.MinimumCostPath(0);
    ui->listWidget_8->clear();
    ui->listWidget_8->addItem(new QListWidgetItem(QString::fromStdString("Node : Travel From")));
    for (int i = 0; i < parent.size(); i++) {
        string s = to_string(i + 1) + " : " + ((parent[i] == -1) ? "Source" : to_string(parent[i] + 1));
        ui->listWidget_8->addItem(new QListWidgetItem(QString::fromStdString(s)));
    }
}
